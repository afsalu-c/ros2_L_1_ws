...
{
public:
   TurtleSpawnerNode() : Node("turtle_spawner"), turtle_counter_(1)
   {
    ...
    spawn_frequncy_ = this->get_parameter("spawn_frequency").as_double();
    ...
    spawn_turtle_timer_ = this->create_wall_timer(
        std::chrono::milliseconds((int)(1000.0 / spawn_frequncy_)),std::bind(&TurtleSpawnerNode::spawnNewTurtle, this));
...
private:
   ...
   void callKillTurtleService(std::string turtle_name)
    ...
    {
         (void)future;//is a common technique in C and C++ used to explicitly mark a variable as intentionally unused, 
   ...
   int turtle_counter_; //int is a built-in type
   double spawn_frequncy_;
   std::string turtle_name_prefix_; //std::string is a class in the C++ Standard Library
