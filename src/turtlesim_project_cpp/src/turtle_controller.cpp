...
#include <cmath> //This is a header file that includes the C standard math library.
...
{
public:
   TurtleControllerNode() : Node("turtle_controller"), name_("turtle1"),
   turtlesim_up_(false)
   {
    ...
    control_loop_timer_ = this->create_wall_timer(
        0.01s,std::bind(&TurtleControllerNode::control_loop,this));
    cmd_vel_publisher_ = this->create_publisher<geometry_msgs::msg::Twist>(name_ + "/cmd_vel", 10);
    pose_subscriber_ = this->create_subscription<turtlesim::msg::Pose>(name_ + "/pose", 10, std::bind(&TurtleControllerNode::callback_pose,this, _1));
    alive_turtles_subscriber_ = this->create_subscription<my_robot_interfaces::msg::TurtleArray>("alive_turtles",10, std::bind(&TurtleControllerNode::callbackAliveTurtles,this, _1));
...
private:
   ...
   {
    pose_ = *pose.get();
    turtlesim_up_ = true; //the variable turtlesim_up_ is a boolean flag used to ensure that control commands are only sent after the first pose message has been received from the turtlesim node.
   }
   ...
   void callbackAliveTurtles(const my_robot_interfaces::msg::TurtleArray::SharedPtr msg)
   {
    if (!msg->turtles.empty())
    {
        ...
        {
            my_robot_interfaces::msg::Turtle closet_turtle = msg->turtles.at(0);
            double closest_turtle_distance = getDistanceFromCurrentPose(closet_turtle);
            ...
            for (int i = 1; i < (int)msg->turtles.size(); i++)
            {
                ...
                {
                    closet_turtle = msg->turtles.at(i);
                    closest_turtle_distance = distance;
                }
            }
        
        turtle_to_catch_ = closet_turtle;
        }
        else 
        {
